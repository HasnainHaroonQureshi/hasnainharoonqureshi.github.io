...
/* ──────────────────────────────────────────────────────────────
   5. WEBGL-LIKE 3D DEPTH CANVAS — gravitational particle field
──────────────────────────────────────────────────────────────── */
(function initDeepCanvas() {
  /* Replace or reuse #anim-canvas */
  ...
  /* ── 3D Depth Particles ── */
  const N_LAYERS = 3;
  const COUNTS   = [30, 50, 80]; // back, mid, front
  const SPEEDS   = [0.15, 0.3, 0.55];
  const RADII    = [0.8, 1.5, 2.5];
  const ALPHAS   = [0.2, 0.45, 0.8];

  const layers = COUNTS.map((count, li) =>
    Array.from({ length: count }, () => ({
      x: rand(0, 1), y: rand(0, 1),
      vx: rand(-1, 1) * SPEEDS[li],
      vy: rand(-1, 1) * SPEEDS[li],
      r: RADII[li],
      phase: rand(0, PI2),
      layer: li,
    }))
  );
  const allParticles = layers.flat();

  /* ── Gravitational Attractors ── */
  const ATTRACTORS = Array.from({ length: 3 }, () => ({
    x: rand(0.2, 0.8), y: rand(0.2, 0.8),
    vx: rand(-0.0004, 0.0004),
    vy: rand(-0.0004, 0.0004),
    strength: rand(0.000018, 0.000032), // much weaker — prevents permanent clumping
  }));

  /* Track cursor movement for blast force */
  let cursorMoving = false, cursorStillTimer = null;
  document.addEventListener('mousemove', () => {
    cursorMoving = true;
    clearTimeout(cursorStillTimer);
    cursorStillTimer = setTimeout(() => { cursorMoving = false; }, 100);
  }, { passive: true });

  /* ── Nebula blobs ── */
  const BLOBS = Array.from({ length: 6 }, (_, i) => ({
    x: rand(0.1, 0.9), y: rand(0.1, 0.9),
    rx: rand(0.12, 0.28), ry: rand(0.10, 0.22),
    phase: rand(0, PI2),
  ...
  function loop() {
    ...
    /* ── Draw nebula blobs (faint atmospheric depth) ── */
    BLOBS.forEach(b => {
      b.x += Math.sin(t * b.speed * 7 + b.phase) * 0.0003;
      b.y += Math.cos(t * b.speed * 5 + b.phase) * 0.0002;
      b.x = (b.x + 1) % 1; b.y = (b.y + 1) % 1;

      const cx = b.x * W, cy = b.y * H;
      const rx = b.rx * W, ry = b.ry * H;
      const grd = ctx.createRadialGradient(cx, cy, 0, cx, cy, rx);
      grd.addColorStop(0, `hsla(${b.hue},80%,55%,0.045)`);
      grd.addColorStop(1, 'transparent');
      ctx.save();
      ctx.scale(1, ry / rx);
      ctx.beginPath();
      ctx.arc(cx, cy * (rx / ry), rx, 0, PI2);
      ctx.fillStyle = grd;
      ctx.fill();
      ctx.restore();
    });

    /* ── Update attractors (drift independently, ignore mouse) ── */
    ATTRACTORS.forEach(a => {
      a.vx *= 0.99; a.vy *= 0.99;
      a.x += a.vx; a.y += a.vy;
      a.x = clamp(a.x, 0.05, 0.95);
      a.y = clamp(a.y, 0.05, 0.95);
      /* Bounce off edges */
    ...
    let hue0 = (190 + t * 8) % 360;

    /* Per-frame cursor speed for blast scaling */
    const cursorSpeedN = Math.min(
      Math.sqrt(MOUSE.vx * MOUSE.vx + MOUSE.vy * MOUSE.vy) / 30, 1
    );

    allParticles.forEach(p => {
      const li = p.layer;

      /* Soft gravity from attractors — clamped at close range so they never trap */
      ATTRACTORS.forEach(a => {
        const dx = a.x - p.x, dy = a.y - p.y;
        const d2 = Math.max(dx*dx + dy*dy, 0.004); // soft-min prevents singularity
        const f  = a.strength / d2 * (li * 0.3 + 0.2);
        p.vx += dx * f; p.vy += dy * f;
      });

      /* ── Inter-particle repulsion — prevents permanent clumping ── */
      allParticles.forEach(q => {
        if (q === p || q.layer !== li) return; // only same-layer
        const dx = p.x - q.x, dy = p.y - q.y;
        const d2 = dx*dx + dy*dy;
        const minD = 0.018; // repulsion radius in normalised coords
        if (d2 < minD * minD && d2 > 0) {
          const d   = Math.sqrt(d2);
          const f   = (minD - d) / minD * 0.00025;
          p.vx += (dx / d) * f;
          p.vy += (dy / d) * f;
        }
      });

      /* ── Mouse blast repulsion — strong, wide, scales with cursor speed ── */
      const dxm = p.x - mx, dym = p.y - my;
      const dm  = Math.sqrt(dxm*dxm + dym*dym) + 0.001;
      const BLAST_R = 0.28; // larger radius
      if (dm < BLAST_R) {
        const proximity = 1 - dm / BLAST_R;           // 0→1 as cursor gets closer
        const speedBoost = 1 + cursorSpeedN * 5;       // up to 6× when cursor is fast
        const f = proximity * proximity * 0.018 * speedBoost;
        p.vx += (dxm / dm) * f;
        p.vy += (dym / dm) * f;
      }

      /* Wander (random thermal energy keeps particles alive) */
      p.vx += (Math.random() - 0.5) * 0.0006;
      p.vy += (Math.random() - 0.5) * 0.0006;

      /* Dampen — back layer damps more, front layer stays lively */
      const damp = [0.970, 0.976, 0.982][li];
      p.vx *= damp; p.vy *= damp;

      /* Speed cap — raised so blasted particles can actually escape */
      const speed  = Math.sqrt(p.vx*p.vx + p.vy*p.vy);
      const maxSpd = SPEEDS[li] * 0.055;
      ...
      const ph = (hue0 + li * 30 + p.phase * 20) % 360;
      const pA = ALPHAS[li] * (0.75 + 0.25 * Math.sin(t * 1.5 + p.phase));
      const pr = p.r * (0.85 + 0.15 * Math.sin(t * 2 + p.phase));

      /* Draw with depth-based glow */
      const glow = ctx.createRadialGradient(px, py, 0, px, py, pr * (li + 1) * 3);
      glow.addColorStop(0,   `hsla(${ph},100%,70%,${pA})`);
      glow.addColorStop(0.5, `hsla(${ph},100%,60%,${pA * 0.3})`);
      glow.addColorStop(1,   'transparent');
      ctx.beginPath();
      ctx.arc(px, py, pr * (li + 1) * 3, 0, PI2);
      ctx.fillStyle = glow;
      ctx.fill();
      ...
      ctx.fillStyle = `hsla(${ph},100%,80%,${pA})`;
      ctx.fill();
    });

    /* ── Draw constellation lines (only between front-layer particles close to each other) ── */
    const frontLayer = layers[2];
    const LINK_DIST  = 0.12;
    for (let i = 0; i < frontLayer.length; i++) {
      for (let j = i + 1; j < frontLayer.length; j++) {
        const a = frontLayer[i], b = frontLayer[j];
        const dx = a.x - b.x, dy = a.y - b.y;
        const d  = Math.sqrt(dx*dx + dy*dy);
        if (d < LINK_DIST) {
          const alpha = (1 - d / LINK_DIST) * 0.2;
          const ph = (hue0 + i * 4) % 360;
          ctx.beginPath();
          ctx.moveTo(a.x*W, a.y*H);
          ctx.lineTo(b.x*W, b.y*H);
          ctx.strokeStyle = `hsla(${ph},100%,70%,${alpha})`;
          ctx.lineWidth   = 0.6;
          ctx.stroke();
        }
      }
    ...
    shootingStars.forEach((s, i) => {
      ...
      grd.addColorStop(1,   `hsla(${s.hue},100%,90%,${s.life})`);
      ctx.beginPath();
  ...
  function spawnShootingStar() {
    ...
    shootingStars.push({
      x: rand(0, W * 0.7), y: rand(0, H * 0.3),
      vx: Math.cos(angle) * rand(6, 14),
